        ...
        for data_name in self.dataset_name:
            ...
            for mode in ["train","test"]:
                ...
                if len(defect_classes) == 1 and "good" in defect_classes and mode=="train":
                    data_list = glob.glob(os.path.join(data_path,mode,"good",'*.tif'))
                    data_list = sorted(data_list)
                    for i in range(len(data_list)):
                        raw_img = io.imread(data_list[i])
                        raw_img = cv2.cvtColor(raw_img, cv2.COLOR_BGR2RGB)
                        img_name_id = os.path.basename(data_list[i]).split(".")[0]
                        new_class_name = self.dataset_map[data_name]
                        save_img_path = os.path.join(des_path_root,new_class_name,"train","good","ClinicDB_"+str(img_name_id)+ "_" + str(id).zfill(6)+".bmp")  
                        cv2.imwrite(save_img_path,raw_img)
                        id = id + 1
                else:
                    if len(defect_classes) >= 1 and mode == "test":
                        for classes in defect_classes:
                            data_list = glob.glob(os.path.join(data_path,mode,classes,'*.tif'))
                            data_list = sorted(data_list)
                            if classes == "good":
                                for i in range(len(data_list)):
                                    img_name_id = os.path.basename(data_list[i]).split(".")[0]
                                    ...
                                    raw_img = cv2.cvtColor(raw_img, cv2.COLOR_BGR2RGB)
                                    new_class_name = self.dataset_map[data_name]
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","good","ClinicDB_"+"good_"+str(img_name_id)+ "_" +str(id).zfill(6)+".bmp") 
                                    cv2.imwrite(save_img_path,raw_img)
                                    id = id + 1
                            else:
                                for i in range(len(data_list)):
                                    ...
                                    new_class_name = self.dataset_map[data_name]
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"ClinicDB_{classes}_"+str(img_name_id)+ "_" +str(id).zfill(6)+".bmp")
                                    save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"ClinicDB_{classes}_"+str(img_name_id)+ "_" +str(id).zfill(6)+".png")
                                    cv2.imwrite(save_img_path,raw_img)
                                    cv2.imwrite(save_mask_path,raw_mask)
                                    id = id + 1
        print("ClinicDB finished !")
        return id 
