            ...
            for mode in ["train","test"]:
                ...
                if len(defect_classes) == 1 and "good" in defect_classes and mode=="train":
                    ...
                    for i in range(len(data_list)):
                        ...
                        save_img_path = os.path.join(des_path_root,new_class_name,"train","good","ISIC_"+str(img_name_id)+ "_" + str(id).zfill(6)+".bmp")  
                        ...
                        for classes in defect_classes:
                            ...
                            if classes == "good":
                                for i in range(len(data_list)):
                                    ...
                                    raw_img = cv2.imread(data_list[i],cv2.IMREAD_COLOR)
                                    new_class_name = self.dataset_map[data_name]
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","good","ISIC_"+"good_"+str(img_name_id)+ "_" +str(id).zfill(6)+".bmp") 
                                    cv2.imwrite(save_img_path,raw_img)
                                    id = id + 1
                            else:
                                for i in range(len(data_list)):
                                    img_name_id = os.path.basename(data_list[i]).split(".")[0]
                                    raw_mask_path = os.path.join(data_path,"ground_truth",classes,os.path.basename(data_list[i]).replace(".jpg","_Segmentation.png"))
                                    raw_img = cv2.imread(data_list[i],cv2.IMREAD_COLOR)
                                    ...
                                    new_class_name = self.dataset_map[data_name]
                                    save_img_path = os.path.join(des_path_root,new_class_name,"test","anomaly",f"ISIC_{classes}_"+str(img_name_id)+ "_" +str(id).zfill(6)+".bmp")
                                    save_mask_path = os.path.join(des_path_root,new_class_name,"ground_truth","anomaly",f"ISIC_{classes}_"+str(img_name_id)+ "_" +str(id).zfill(6)+".png")
                                    cv2.imwrite(save_img_path,raw_img)
